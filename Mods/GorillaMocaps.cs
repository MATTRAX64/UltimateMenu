...
namespace UltimateMenu
{
    internal class GorillaMocaps : MonoBehaviour
    {
        ...
        /// Quand désactivé, l'enregistrement en cours est arrêté/sauvegardé et le gizmo d'origine
        /// est masqué (sans perdre la calibration) dans l'interface.
        /// </summary>
        ...
        void Update()
        {
            ...
            // Détecte l'activation/désactivation du mod : arrête proprement tout enregistrement
            // en cours (pour ne pas laisser un fichier à moitié écrit ou un thread orphelin) et
            // masque/réaffiche le gizmo d'origine en conséquence.
            ...
            if (nowEnabled != _wasEnabled)
            {
                ...
                if (!nowEnabled)
                {
                    ...
                    // Masque le gizmo 3D de l'origine quand le mod est désactivé, sans perdre
                    // la calibration (originSet/_originPos restent intacts).
                    if (_originMarker != null) _originMarker.SetActive(false);
                }
                else
                {
                    // Réaffiche le gizmo si une origine était déjà calibrée.
                    if (_originMarker != null) _originMarker.SetActive(true);
                }
            }
        }
        ...
        void TryRegister()
        {
            ...
            {
                MenuUI.Instance.RegisterPage("Mocap.Title", DrawPage, MenuCategory.Mocap);
        ...
        static Type? GetPhotonNetworkType()
        {
            ...
            foreach (var asm in AppDomain.CurrentDomain.GetAssemblies())
            {
                ...
                foreach (var t in types)
                    if (t.Name == "PhotonNetwork") { _photonNetworkType = t; return t; }
            }
            return null;
        }
        ...
        bool PhotonInRoom()
        {
            var t = GetPhotonNetworkType();
            var p = t?.GetProperty("InRoom", BindingFlags.Public | BindingFlags.Static);
            return p != null && (bool)(p.GetValue(null) ?? false);
        }

        string PhotonRoomName()
        {
            var t = GetPhotonNetworkType();
            var room = t?.GetProperty("CurrentRoom", BindingFlags.Public | BindingFlags.Static)?.GetValue(null);
            if (room == null) return "NoRoomCode";
            return room.GetType().GetProperty("Name")?.GetValue(room) as string ?? "NoRoomCode";
        }

        PhotonPlayer[] PhotonPlayerList()
        {
            var t = GetPhotonNetworkType();
            var list = t?.GetProperty("PlayerList", BindingFlags.Public | BindingFlags.Static)?.GetValue(null) as PhotonPlayer[];
            return list ?? Array.Empty<PhotonPlayer>();
        }

        PhotonPlayer? PhotonLocalPlayer()
        {
            var t = GetPhotonNetworkType();
            return t?.GetProperty("LocalPlayer", BindingFlags.Public | BindingFlags.Static)?.GetValue(null) as PhotonPlayer;
        }
        ...
        void RefreshPlayers()
        {
            ...
            if (localRig != null)
            {
                ...
                _players.Add(new PlayerEntry
                {
                    ...
                    rig = localRig,
                    selected = oldSelection.TryGetValue(realName, out bool sel) ? sel : true,
                    displayLabel = "(Moi) " + realName,
                    dataName = realName
                });
            }
            ...
            if (PhotonInRoom() && GorillaGameManager.instance != null)
            {
                foreach (var p in PhotonPlayerList())
                {
                    ...
                    _players.Add(new PlayerEntry
                    {
                        photonPlayer = p,
                        rig = rig,
                        selected = oldSelection.TryGetValue(p.NickName, out bool sel) ? sel : false,
                        displayLabel = p.NickName,
                        dataName = p.NickName
                    });
                }
            }
        ...
        {
            if (_autoOriginDone || originSet) return;
        ...
        void SetOrigin()
        {
            ...
            // Si le mod est désactivé au moment où l'origine est (re)calibrée, le gizmo
            // fraîchement créé doit rester masqué pour respecter l'état visuel attendu.
        ...
        void ClearOrigin()
        {
            ...
            if (_originMarker != null) Destroy(_originMarker);
            _originMarker = null;
            SetStatus(I18n.T("Mocap.OriginCleared"));
        }
        ...
        GameObject BuildOriginGizmo()
        {
            ...
            var col = center.GetComponent<Collider>();
        ...
        public void StartRecording()
        {
            ...
            // Génère un nouveau nom de fichier unique pour cet enregistrement, pour ne jamais
            // écraser un enregistrement précédent fait dans la même room.
            NewFileGroup(PhotonInRoom() ? PhotonRoomName() : "NoRoomCode");
        ...
        void RefreshCamera()
        {
            ...
            {
                _lckCamera = found;
                _camStatus = I18n.T("Mocap.CamFound");
            }
            else
            {
                _lckCamera = null;
                _camStatus = I18n.T("Mocap.CamNotFound");
            }
        }
        ...
        public void SnapCameraToPlayer()
        {
            ...
            var rig = VRRig.LocalRig;
            var head = rig != null ? rig.transform.Find("rig/head") : null;
        ...
        void UpdateSnapTest()
        {
            ...
            if (_snapTimer <= 0f && _lckCamera != null)
            {
                _lckCamera.transform.SetPositionAndRotation(_snapSavedPos, _snapSavedRot);
                _snapActive = false;
                SetStatus(I18n.T("Mocap.CamRestored"));
            }
        }
        ...
        void FixedUpdate()
        {
            ...
            if (recording)
            {
                ...
                {
                    if (!p.selected || p.rig == null) continue;
                    StorePlayerData(p);
                }
            }
        ...
        bool SampleReady(ref float timer, float rate)
        {
            ...
            float interval = 1f / Mathf.Max(rate, 1f);
            if (timer >= interval) { timer = 0f; return true; }
            return false;
        }

        void StorePlayerData(PlayerEntry p)
        {
            if (p.rig == null) return;
            if (!SampleReady(ref p.sampleTimer, sampleRate)) return;

            int actorNum = p.photonPlayer != null ? p.photonPlayer.ActorNumber : 0;
            string key = actorNum + "_" + p.dataName;
            int idx = _dataList.FindIndex(d => d.name == key);
            float lastSeen = Time.time - _recordingStartTime;

            var head = p.rig.transform.Find("rig/head");
            var handL = p.rig.transform.Find("rig/hand.L");
            var handR = p.rig.transform.Find("rig/hand.R");
            var body = p.rig.transform.Find("rig/body_pivot/body/body_new");
            var parts = new[] { head, handL, handR, body };

            if (idx == -1)
            {
                var data = new GorillaMocapPlayerData(key, sampleRate, GetColor(p.rig), 4, lastSeen);
                _dataList.Add(data);
                idx = _dataList.Count - 1;
            }
            ...
            for (int i = 0; i < parts.Length; i++)
            {
                var t = parts[i];
                if (t == null) continue;
                _dataList[idx].objects[i].locations.Add(ApplyOrigin(t.position));
                _dataList[idx].objects[i].rotations.Add(RelativeToOrigin(t.rotation));
            }

            bool isLocal = p.photonPlayer != null && p.photonPlayer.IsLocal;
            if (isLocal)
            {
        ...
        int DetectSkin(VRRig rig)
        {
            try
            {
                var tex = rig.mainSkin != null && rig.mainSkin.material != null ? rig.mainSkin.material.mainTexture : null;
                return tex != null && tex.name == "lavasmall" ? 1 : 0;
            }
            catch { return 0; }
        }

        Color GetColor(VRRig rig)
        {
            try { return rig.mainSkin.material.color; } catch { return Color.white; }
        }
        ...
        void StoreCameraLive()
        {
            ...
            if (_snapActive) return;
            if (_lckCamera == null || !_lckCamera.activeInHierarchy)
            {
                cameraRecording = false;
                SaveSingleFileAsync(_camData);
                _camData = null;
                SetStatus(I18n.T("Mocap.CamLost"));
                return;
            }
            ...
            Quaternion relRot = RelativeToOrigin(camT.rotation);
            _camData.objects[0].rotations.Add(relRot * CameraRotationOffset);
        }

        void CreateDataFolder()
        {
            string dllDir = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location) ?? Directory.GetCurrentDirectory();
            _mocapFolder = Path.Combine(dllDir, "GorillaMocap");
            if (!Directory.Exists(_mocapFolder)) Directory.CreateDirectory(_mocapFolder);
        }

        /// <summary>
        /// Extrait GorillaMocap-BlenderAsset.zip depuis les ressources embarquées du DLL vers le
        /// dossier GorillaMocap, uniquement si le fichier n'existe pas déjà sur le disque. Ce package
        /// aide l'utilisateur à importer les enregistrements mocap dans Blender.
        /// </summary>
        void EnsureBlenderAssetExtracted()
        {
            ...
            try
            {
                ...
                using (var stream = asm.GetManifestResourceStream(BLENDER_ASSET_RESOURCE))
                {
                    if (stream == null)
                    {
                        Debug.LogError("[GorillaMocap] Ressource embarquée introuvable : " + BLENDER_ASSET_RESOURCE +
                                       ". Vérifie que GorillaMocap-BlenderAsset.zip est bien marqué 'Embedded Resource' dans le projet.");
                        return;
                    }
            ...
            {
                Debug.LogError("[GorillaMocap] EnsureBlenderAssetExtracted: " + e);
            }
        }

        public void NewFileGroup(string roomCode)
        {
            if (string.IsNullOrEmpty(_mocapFolder)) CreateDataFolder();
            string safeRoom = string.IsNullOrEmpty(roomCode) ? "NoRoomCode" : roomCode;
            _roomFileCode = safeRoom + "_" + DateTime.Now.ToString("yyyy-MM-dd_HH-mm-ss");
        }
        ...
        void WritePlayerFile(string path, GorillaMocapPlayerData data)
        {
            ...
            sb.Append('#').Append("2.1.8").Append('$')
              .Append(data.framerate).Append('$')
              .Append(data.name).Append('$')
              .Append(data.color.ToString()).Append('$')
              .Append(data.startTime).Append('$')
              .Append(data.lastSeenTime).Append('$')
              .Append(data.isCamera ? "camera" : "player").Append('\n');

            sb.Append('#').Append("room=").Append(PhotonInRoom() ? PhotonRoomName() : "Hors ligne").Append('$')
              .Append("date=").Append(DateTime.Now.ToString("dd/MM/yyyy HH:mm:ss")).Append('$')
              .Append("origin=").Append(originSet ? "1" : "0").Append('\n');
            ...
            var first = data.objects[0];
            var fileSb = new StringBuilder();
            for (int j = 0; j < first.locations.Count; j++)
            {
                lineBuilder.Clear();
                for (int o = 0; o < data.objects.Count; o++)
                {
                    ...
                    lineBuilder.Append(obj.locations[j].ToString("F3")).Append('~').Append(obj.rotations[j].ToString("F3"));
                }
                if (!data.isCamera && data.fingers[0].values.Count > 0)
                    foreach (var f in data.fingers) lineBuilder.Append('$').Append(f.values[j].ToString("F1"));
                if (!data.isCamera && data.skinIndex.values.Count > j)
                    lineBuilder.Append('$').Append(data.skinIndex.values[j].ToString("F0"));
        ...
        void DrawPage(MenuUI ui)
        {
            ...
            if (recording)
            {
                ...
                ui.Label(FormatTime(elapsed) + "  -  " + _totalSamples + " " + I18n.T("Mocap.Samples"), Theme.Accent, 10, true);
            }
            ui.Space(4);
            if (ui.Btn(recording ? I18n.T("Mocap.StopSave") : I18n.T("Mocap.StartRecord"), recording ? ui.TR : ui.TA, Color.white) && Enabled)
            ...
            _fpsInput = GUILayout.TextField(_fpsInput, GUILayout.Width(60));
            ...
            {
                GUILayout.BeginHorizontal();
                bool newSel = GUILayout.Toggle(p.selected, "");
            ...
            ui.Space(4);
            GUILayout.BeginHorizontal();
            bool newCamSel = GUILayout.Toggle(cameraSelected, "");
            ...
            if (cameraSelected)
            {
                ...
                GUILayout.BeginHorizontal();
                _camFpsInput = GUILayout.TextField(_camFpsInput, GUILayout.Width(60));
            ...
            // Les coordonnées détaillées de l'origine ne sont affichées que si le mod est activé.
            ...
            if (_lastExportedFiles.Count > 0)
            {
                ...
                ui.BeginCard();
                foreach (var f in _lastExportedFiles)
                    ui.Label(Path.GetFileName(f), Theme.Accent, 9, true);
    ...
    internal class GorillaMocapObject
    {
