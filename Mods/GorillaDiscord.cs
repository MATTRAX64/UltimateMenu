...
namespace UltimateMenu
{
    public class GorillaDiscord : MonoBehaviour
    {
        ...
        /// Quand désactivé, la connexion Discord (pipe IPC) est coupée pour ne pas laisser
        /// le Rich Presence tourner en arrière-plan alors que le mod est censé être éteint.
        /// </summary>
        ...
        void Update()
        {
            ...
            // Détecte la désactivation du mod : coupe la connexion Discord proprement.
        ...
        void RefreshAndSend()
        {
            ...
            {
                var f = typeof(GorillaRooms).GetField("currentRoomCode", BindingFlags.NonPublic | BindingFlags.Instance);
                _currentRoom = f?.GetValue(GorillaRooms.Instance)?.ToString() ?? "";
            }
