...
namespace UltimateMenu
{
    ...
    public static class Trad
    {
        ...
        public static void Register()
        {
            ...
            R("Core.home_author", "Par MATTRAXER64xp", "By MATTRAXER64xp", "Von MATTRAXER64xp", "Por MATTRAXER64xp", "Di MATTRAXER64xp", "Por MATTRAXER64xp", "作者: MATTRAXER64xp", "Автор: MATTRAXER64xp");
            ...
            R("Filter.Title", "Filtre Visuel", "Visual Filter", "Visueller Filter", "Filtro visual", "Filtro visivo", "Filtro visual", "ビジュアルフィルター", "Визуальный фильтр");
            ...
            R("TimeWeather.Locked", "Verrouillé", "Locked", "Gesperrt", "Bloqueado", "Bloccato", "Bloqueado", "ロック中", "Заблокировано");
            R("TimeWeather.Unlocked", "Libre", "Free", "Frei", "Libre", "Libero", "Livre", "自由", "Свободно");
            ...
            R("TimeWeather.RainOn", "Pluie activée", "Rain enabled", "Regen aktiviert", "Lluvia activada", "Pioggia attiva", "Chuva ativada", "雨 有効", "Дождь включён");
            R("TimeWeather.RainOff", "Pluie désactivée", "Rain disabled", "Regen deaktiviert", "Lluvia desactivada", "Pioggia disattivata", "Chuva desativada", "雨 無効", "Дождь выключен");
            R("TimeWeather.Enable", "Activer", "Enable", "Aktivieren", "Activar", "Attiva", "Ativar", "有効化", "Включить");
            R("TimeWeather.Disable", "Désactiver", "Disable", "Deaktivieren", "Desactivar", "Disattiva", "Desativar", "無効化", "Выключить");
            ...
            R("Filter.Section", "Filtre visuel", "Visual Filter", "Visueller Filter", "Filtro visual", "Filtro visivo", "Filtro visual", "ビジュアルフィルター", "Визуальный фильтр");
            R("Filter.Enable", "Activer le filtre", "Enable filter", "Filter aktivieren", "Activar filtro", "Attiva filtro", "Ativar filtro", "フィルターを有効化", "Включить фильтр");
            R("Filter.Settings", "Réglages manuels", "Manual settings", "Manuelle Einstellungen", "Ajustes manuales", "Impostazioni manuali", "Configurações manuais", "手動設定", "Ручные настройки");
            R("Filter.Brightness", "Luminosité", "Brightness", "Helligkeit", "Brillo", "Luminosità", "Brilho", "明るさ", "Яркость");
            R("Filter.Warmth", "Chaleur (froid <-> chaud)", "Warmth (cold <-> warm)", "Wärme (kalt <-> warm)", "Calor (frío <-> cálido)", "Calore (freddo <-> caldo)", "Calor (frio <-> quente)", "暖かさ (寒い <-> 暖かい)", "Теплота (холодно <-> тепло)");
            R("Filter.Vignette", "Vignette (bords sombres)", "Vignette (dark edges)", "Vignette (dunkle Ränder)", "Viñeta (bordes oscuros)", "Vignetta (bordi scuri)", "Vinheta (bordas escuras)", "ビネット (暗い縁)", "Виньетка (тёмные края)");
            ...
            // === GorillaInterface (menu VR world-space, joystick + trigger) ===
            R("GorillaInterface.Title", "Interface VR", "VR Interface", "VR-Schnittstelle", "Interfaz VR", "Interfaccia VR", "Interface VR", "VRインターフェース", "VR-интерфейс");
            R("GorillaInterface.Section", "Interface VR (menu en jeu)", "VR Interface (in-game menu)", "VR-Schnittstelle (Ingame-Menü)", "Interfaz VR (menú en el juego)", "Interfaccia VR (menu in gioco)", "Interface VR (menu no jogo)", "VRインターフェース（ゲーム内メニュー）", "VR-интерфейс (меню в игре)");
            R("GorillaInterface.Desc", "Menu en jeu pour gérer les mods | Joystick : Bas → Haut = ouvrir | Haut → Bas = fermer.", "In-game menu to manage mods | Joystick: Down → Up = open | Up → Down = close.", "In-Game-Menü zum Verwalten von Mods | Joystick: Unten → Oben = öffnen | Oben → Unten = schließen.", "Menú en el juego para gestionar los mods | Joystick: Abajo → Arriba = abrir | Arriba → Abajo = cerrar.", "Menu di gioco per gestire le mod | Joystick: Basso → Alto = aprire | Alto → Basso = chiudere.", "Menu no jogo para gerenciar mods | Joystick: Baixo → Cima = abrir | Cima → Baixo = fechar.", "MODを管理するゲーム内メニュー | ジョイスティック：下 → 上 = 開く | 上 → 下 = 閉じる。", "Внутриигровое меню для управления модами | Джойстик: Вниз → Вверх = открыть | Вверх → Вниз = закрыть.");
            R("GorillaInterface.Active", "Actif", "Active", "Aktiv", "Activo", "Attivo", "Ativo", "アクティブ", "Активно");
            R("GorillaInterface.Inactive", "Inactif", "Inactive", "Inaktiv", "Inactivo", "Inattivo", "Inativo", "非アクティブ", "Неактивно");

            R("GorillaInterface.SizeLabel", "Taille du menu", "Menu size", "Menügröße", "Tamaño del menú", "Dimensione del menu", "Tamanho do menu", "メニューのサイズ", "Размер меню");
            R("GorillaInterface.SizeShort", "Échelle", "Scale", "Skalierung", "Escala", "Scala", "Escala", "スケール", "Масштаб");

            R("GorillaInterface.DistanceLabel", "Distance d'apparition", "Spawn distance", "Erscheinungsdistanz", "Distancia de aparición", "Distanza di comparsa", "Distância de aparecimento", "出現距離", "Дистанция появления");
            R("GorillaInterface.DistanceShort", "Distance", "Distance", "Distanz", "Distancia", "Distanza", "Distância", "距離", "Дистанция");

            R("GorillaInterface.HeightLabel", "Décalage vertical", "Vertical offset", "Vertikaler Versatz", "Desplazamiento vertical", "Offset verticale", "Deslocamento vertical", "垂直方向のオフセット", "Вертикальное смещение");
            R("GorillaInterface.HeightShort", "Hauteur", "Height", "Höhe", "Altura", "Altezza", "Altura", "高さ", "Высота");

            R("GorillaInterface.AnimSpeedLabel", "Vitesse d'animation (ouverture/fermeture)", "Animation speed (open/close)", "Animationsgeschwindigkeit (öffnen/schließen)", "Velocidad de animación (abrir/cerrar)", "Velocità animazione (apertura/chiusura)", "Velocidade da animação (abrir/fechar)", "アニメーション速度（開閉）", "Скорость анимации (открытие/закрытие)");
            R("GorillaInterface.AnimSpeedShort", "Vitesse", "Speed", "Geschwindigkeit", "Velocidad", "Velocità", "Velocidade", "速度", "Скорость");

            R("GorillaInterface.FollowLabel", "Vitesse de suivi du joueur", "Player follow speed", "Verfolgungsgeschwindigkeit", "Velocidad de seguimiento", "Velocità di inseguimento", "Velocidade de acompanhamento", "追従速度", "Скорость следования");
            R("GorillaInterface.FollowShort", "Suivi", "Follow", "Verfolgung", "Seguimiento", "Inseguimento", "Acompanhamento", "追従", "Следование");

            R("GorillaInterface.MaxDistLabel", "Distance max avant fermeture auto", "Max distance before auto-close", "Max. Distanz vor automatischem Schließen", "Distancia máxima antes de cierre automático", "Distanza massima prima della chiusura automatica", "Distância máxima antes do fechamento automático", "自動終了までの最大距離", "Макс. дистанция до автозакрытия");
            R("GorillaInterface.MaxDistShort", "Max", "Max", "Max", "Máx", "Max", "Máx", "最大", "Макс");

            R("GorillaInterface.StickSensLabel", "Sensibilité du joystick", "Joystick sensitivity", "Joystick-Empfindlichkeit", "Sensibilidad del joystick", "Sensibilità del joystick", "Sensibilidade do joystick", "ジョイスティックの感度", "Чувствительность джойстика");
            R("GorillaInterface.StickSensShort", "Seuil", "Threshold", "Schwelle", "Umbral", "Soglia", "Limite", "しきい値", "Порог");

            R("GorillaInterface.ShowPCTest", "Afficher les boutons de test PC", "Show PC test buttons", "PC-Testschaltflächen anzeigen", "Mostrar botones de prueba en PC", "Mostra i pulsanti di test PC", "Mostrar botões de teste no PC", "PCテストボタンを表示", "Показать тестовые кнопки ПК");
        }
    }
}