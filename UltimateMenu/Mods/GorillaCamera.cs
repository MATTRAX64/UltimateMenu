...
namespace UltimateMenu
{
    public class GorillaCamera : MonoBehaviour
    {
        ...
        {
            if (!_labelMapCache.TryGetValue(layout, out var map)) { map = BuildLabelToPhysical(layout); _labelMapCache[layout] = map; }
        ...
        static Key KeyFor(MoveDir dir, Layout layout) => KCtoKeyStatic(PhysicalForLabel(LabelFor(dir, layout), layout)) ?? Key.None;

        static Key? KCtoKeyStatic(KeyCode kc) => (kc >= KeyCode.A && kc <= KeyCode.Z) ? Key.A + (kc - KeyCode.A) : (Key?)null;
        ...
        void SaveSpeedSens() { PlayerPrefs.SetFloat("MUM_CamSpeed", speed); PlayerPrefs.SetFloat("MUM_CamSens", sens); PlayerPrefs.Save(); }
        void SaveLoop() { PlayerPrefs.SetInt("MUM_CamLoop", LoopKeyframes ? 1 : 0); PlayerPrefs.Save(); }
        ...
        void Update()
        {
            ...
            if (nowEnabled != _wasEnabled)
            {
                ...
                if (nowEnabled) SpawnCameraClone(); else DestroyCameraClone();
            }
        ...
        public void SpawnCameraClone() { if (_state == CamState.None) TriggerSpawn(); }
        ...
        void DelayedFind() { FindCamera(); if (_state == CamState.Finding) Invoke(nameof(DelayedFind), 0.5f); }

        void FindCamera()
        {
            var found = GameObject.Find("LCKTablet(Clone)");
            if (found != null) GrabCamera(found);
        }
        ...
        void Move(Keyboard kb)
        {
            ...
            Key fwd = KeyFor(MoveDir.Forward, layout), bck = KeyFor(MoveDir.Back, layout), lft = KeyFor(MoveDir.Left, layout);
            Key rgt = KeyFor(MoveDir.Right, layout), up = KeyFor(MoveDir.Up, layout), dwn = KeyFor(MoveDir.Down, layout);
        ...
        void DrawPage(MenuUI ui)
        {
            ...
            if (Enabled && (!Mathf.Approximately(newSpeed, speed) || !Mathf.Approximately(newSens, sens))) { speed = newSpeed; sens = newSens; SaveSpeedSens(); }
            ui.Space(6); ui.Separator(); ui.Space(6);

            ui.Label(
                I18n.T("Camera.MoveKeysLabel") + "  " +
                DisplayFor(MoveDir.Forward, layout) + " " + DisplayFor(MoveDir.Left, layout) + " " + DisplayFor(MoveDir.Back, layout) + " " + DisplayFor(MoveDir.Right, layout) + "  |  " +
                DisplayFor(MoveDir.Up, layout) + "/" + DisplayFor(MoveDir.Down, layout) + " " + I18n.T("Camera.UpDownSuffix"),
                Theme.Dim, 9);
            ui.Space(4);
            ...
            else if (ui.RebindBtn(I18n.T("Camera.AddKF"), keyAddKF) && Enabled) StartLocalRebind("CamAddKF");
            ui.EndCard();
            ...
            if (Keyframes.Count == 0) ui.Label(I18n.T("Camera.NoKeyframes"), Theme.Dim, 9);
                ...
                for (int i = 0; i < Keyframes.Count; i++)
                {
                    ...
                    if (ui.Btn((sel ? "> " : "   ") + kf.Label + " - " + kf.Duration.ToString("F1") + "s - " + kf.Ease, sel ? ui.TA : ui.TB3, sel ? Color.white : Theme.Text) && Enabled) selectedKF = i;
                    GUILayout.Space(4);
            ...
            ui.Space(6);
            if (ui.Btn(IsPlayingKeyframes ? I18n.T("Camera.Stop") : I18n.T("Camera.Start"), IsPlayingKeyframes ? ui.TR : ui.TA, Color.white) && Enabled) { if (IsPlayingKeyframes) StopPlay(); else StartPlay(); }
            ui.EndCard();
        }
