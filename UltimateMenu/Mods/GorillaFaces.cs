...
namespace UltimateMenu
{
    public class GorillaFaces : MonoBehaviour
    {
        ...
        void EnsureDefaultFaceExtracted()
        {
            ...
            try
            {
                using (var stream = Assembly.GetExecutingAssembly().GetManifestResourceStream(DEFAULT_FACE_RESOURCE))
                {
                    if (stream == null) { Log.Err("GorillaFaces.EnsureDefaultFaceExtracted", new Exception("Ressource embarquée introuvable : " + DEFAULT_FACE_RESOURCE)); return; }
            ...
            catch (Exception e) { Log.Err("GorillaFaces.EnsureDefaultFaceExtracted", e); }
        }
        void RefreshFileList()
        {
            ...
            if (!string.IsNullOrEmpty(_currentAppliedName))
                _selectedIndex = _availableFiles.IndexOf(Path.Combine(_dataFolder, _currentAppliedName));
        }
        void FindRig()
        {
        ...
        void DrawPage(MenuUI ui)
        {
            ...
            if (filtered.Count == 0) ui.Label(I18n.T("Face.NoImages"), Theme.Dim, 9);
            else
            {
                foreach (var f in filtered)
                {
                    string fname = Path.GetFileName(f);
                    bool applied = fname == _currentAppliedName, isDefault = fname == DEFAULT_FACE_FILENAME;
                    if (ui.Btn(fname + (isDefault ? " (" + I18n.T("Face.DefaultTag") + ")" : ""), applied ? ui.TG : ui.TB3, applied ? Color.white : Theme.Text) && Enabled)
                    {
